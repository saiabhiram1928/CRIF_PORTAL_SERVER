  ...
  try {
    ...
    pool.getConnection(async (err, connection) => {
      ...
      const [rows] = await connection
        .promise()
        .query("SELECT * FROM icpoes WHERE email = ?", [email])
        .catch((err) => {
          if (!res.headersSent) {
            res.status(404).json(err);
          }
...
const getItem = async (req, res, next) => {
  try {
    ...
    pool.getConnection(async (err, connection) => {
      ...
      const [rows] = await connection
        .promise()
        .query("SELECT * FROM icpoes WHERE ref_id = ?", [ref_id])
        .catch((err) => {
          if (!res.headersSent) {
            res.status(404).json(err);
          }
...
const createItem = async (req, res, next) => {
  try {
    ...
    const expected_wavelength = req.body.expected_wavelength;
    const expected_concentration = req.body.expected_concentration;
    ...
    pool.getConnection(async (err, connection) => {
      ...
      await connection
        .promise()
        .query("INSERT INTO icpoes VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
          ref_id,
